...
use Evrinoma\AddressBundle\Factory\Address\Factory as AddressFactory;
use Evrinoma\AddressBundle\Mediator\QueryMediatorInterface;
use Evrinoma\AddressBundle\Repository\Address\AddressCommandRepositoryInterface;
...
class EvrinomaAddressExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\AddressBundle\Entity';
    public const MODEL = 'Evrinoma\AddressBundle\Model';
    public const ENTITY_FACTORY_ADDRESS = AddressFactory::class;
    public const ENTITY_BASE_ADDRESS = BaseAddress::class;
    public const DTO_BASE_ADDRESS = AddressApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_ADDRESS !== $config['factory']) {
        ...
        $this->wireForm($container, $config['dto'], 'address', 'address');
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, AddressPass::ADDRESS_CONSTRAINT)   :
                    $definition->addTag(AddressPass::ADDRESS_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([AddressCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([AddressQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaAddressBundle::BUNDLE;
    }
}
